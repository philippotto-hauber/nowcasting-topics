...
df_ma %>% 
  pivot_longer(cols = -date, names_to = "topics", values_to = "vals") %>%
  mutate(series = "moving average") -> df_ma

df_bw <- as.data.frame(dat_bw)
df_bw$date <- df_topics$date
df_bw %>% 
  pivot_longer(cols = -date, names_to = "topics", values_to = "vals") %>%
  mutate(series = "bw filter") -> df_bw
...
df_plot$series = factor(df_plot$series,levels(df_plot$series)[c(4, 3, 1, 2)])
```
...
ggplot(filter(df_plot, topics %in% c("T0", "T1", "T2", "T3", "T4", "T5", "T11", "T20")), 
  ...
  scale_color_manual(name = element_blank(), 
                     values = c("bw filter" = "blue4", 
                                "moving average" = "blue4",
                                "original" = "gray80",
                                "final" = "darkorange3"))+
  scale_linetype_manual(name = element_blank(), 
                        values = c("bw filter" = "dotted", 
                                   "moving average" = "solid", 
                                   "original" = "solid",
                                   "final" = "solid"))+
  scale_alpha_manual(name = element_blank(),
                     guide = 'none',
                     values = c("bw filter" = 1, 
                                "moving average" = 1, 
                                "original" = 0.4,
                                "final" = 1))+
  labs(x = "", y = "", 
       title = "Transformation of topics series",
       caption = paste0("final = smoothed and detrended series. Note that the range of the y-axes is restricted to [", ylim_lowr, ", ", ylim_uppr, "] for better visibility of the smoothed and detrended series."))+
...
Save plot as pdf
```{r}
#ggsave(paste0(dir_local, "/eval/figures/plot_forecasts_actuals.pdf"), width = 20, height = 15, units = "cm")
```

...
This notebook illustrates how we transform the raw daily time series of topics before estimating the daily factor model. Additionally, it also calculates principal component estimates of transformed series and analyzes their correlation with quarterly GDP.  
...
I personally favor 1) or 2). ~~What does Thorsrud/the literature do?~~ He linearly interpolates the topics on Sundays and estimate the model with a full 7-day week. I am not 100 percent satisfied with that, as the additional missings on Sundays (or bank holidays) don't really pose a challenge from the point of view of estimation.  
...
df_gdp %>% 
  ...
  mutate(year = as.numeric(substr(date, 1, 4)), 
         ...
         quarter = ceiling(month/3),
         date = make_date(year = year, month = month + 1, day = 15) # middle of the quarter, e.g. 15.2. for Q1
         )-> df_gdp
...
df_gdp$d_gdp <- c(NA, 100 * diff(log(df_gdp$gdp)))

# filter to match sample of transformed topics
df_gdp <- df_gdp[df_gdp$year >= 1994 & df_gdp$year <= 2018, ]
```
...
```{r}
cor_topics_gdp <- cor(cbind(as.matrix(df_trafo_Q[, -c(1:2)]), df_gdp$d_gdp))
...
cbind(data.frame(date = df_topics$date), scale(dat_trafo))  %>% 
  ...
  filter(date >= "1996-01-01" & date <= "2018-12-31") -> df_plot_topics

df_gdp %>% 
  mutate(d_gdp_stand = (d_gdp - mean(d_gdp, na.rm = T)) / sd(d_gdp, na.rm = T)) %>%
  filter(date >= "1996-01-01" & date <= "2018-12-31") -> df_plot_gdp
...
ggplot()+
  ...
  labs(y = "standard deviation", x = "", title = "Transformed topics and economic activity",
       subtitles = "5 topics with the highest absolute correlation between quarterly GDP growth",

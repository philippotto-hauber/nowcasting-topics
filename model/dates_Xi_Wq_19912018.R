...
for (y in years)
{
  ...
  for (q in quarters)
  {
    ...
    print(paste("t =", t))
    s <- seq(t_prev + 1, t)
    #W_q_c = k * (t + 1 - s) / k_t # see Banbura et al. (2011, p. 30, eqn 10)
    W_q_c = (t + 1 - s) / k_t # quarterly GDP level  is the average of daily GDP in the respective quarter
    if (y == years[length(years)] & q == quarters[length(quarters)]) # W_q_p = 0 => Not necessary but conceptually clearer!
    {

      df_W_q <- rbind(df_W_q, data.frame(year = y,
                                         quarter = q,
                                         day = seq(1, k_t),
                                         W_q_c = W_q_c,
                                         W_q_p = 0
    ...
    } else
    {
      df_W_q <- rbind(df_W_q, data.frame(year = y,
                                         ...
                                         day = seq(1, k_t),
                                         W_q_c = W_q_c,
                                         W_q_p = c(0, rev(W_q_c[2:length(W_q_c)]))
                                         )
                      )
    }
  }
